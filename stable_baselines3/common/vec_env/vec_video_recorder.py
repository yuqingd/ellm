...
from gym.wrappers.monitoring.video_recorder import ImageEncoder, VideoRecorder
...
class VecVideoRecorder(VecEnvWrapper):
    ...
    def start_video_recorder(self) -> None:
        ...
        self.video_recorder = VideoRecorderCustom(
...
class VideoRecorderCustom(VideoRecorder):

    def _encode_image_frame(self, frame):
        if not self.encoder:
            self.encoder = ImageEncoderCustom(
...
class ImageEncoderCustom(ImageEncoder):
    ...
    def close(self):
        ...
        if m:
            ...
            key = "videos"
        data = np.transpose(self.frames, (0, 3, 1, 2))
        wandb.log({key: wandb.Video(data)})
        self.frames = []

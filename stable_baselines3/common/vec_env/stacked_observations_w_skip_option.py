...
class StackedObservations(object):
    ...
    def __init__(
        ...
        (
            ...
            self.stackedobs,
            self.repeat_axis,
        ) = self.compute_stacking(num_envs, n_stack, n_skip, observation_space, channels_order)
        super().__init__()
    ...
    def compute_stacking(
        ...
        stackedobs = np.zeros((n_skip , num_envs,) + low.shape, low.dtype)
    ...
    def reset(self, observation: np.ndarray) -> np.ndarray:
        ...
        if self.channels_first:
            ...
            self.stackedobs[..., -observation.shape[self.stack_dimension] :] = observation
        skipidx = 0  # simply return the first skipidx, as they are all the same
        return self.stackedobs[skipidx]
    ...
    def update(
        ...
        """
        # Determine index of the stackedobs we put the new observation in
        skipidx = self.obs_cnt % self.n_skip
        self.obs_cnt += 1
        stack_ax_size = observations.shape[self.stack_dimension]
        self.stackedobs[skipidx] = np.roll(self.stackedobs[skipidx], shift=-stack_ax_size, axis=self.stack_dimension)
        for i, done in enumerate(dones):
            if done:
                if "terminal_observation" in infos[i]:
                    ...
                    if self.channels_first:
                        new_terminal = np.concatenate(
                            (self.stackedobs[skipidx, i, :-stack_ax_size, ...], old_terminal),
                ...
                self.stackedobs[skipidx, i] = 0
        if self.channels_first:
            self.stackedobs[skipidx, :, -observations.shape[self.stack_dimension] :, ...] = observations
        else:
            self.stackedobs[..., -observations.shape[self.stack_dimension] :] = observations
        return self.stackedobs[skipidx], infos

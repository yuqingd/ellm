from copy import deepcopy
from typing import Dict, Union

import numpy as np
from gym import spaces

from stable_baselines3.common.preprocessing import is_image_space, is_image_space_channels_first
from stable_baselines3.common.vec_env.base_vec_env import VecEnv, VecEnvStepReturn, VecEnvWrapper


class VecNormalizeImage(VecEnvWrapper):
    """
    Convert to float, and normalize image to [-1, 1] range.

    :param venv:
    :param skip: Skip this wrapper if needed as we rely on heuristic to apply it or not,
        which may result in unwanted behavior, see GH issue #671.
    """

    def __init__(self, venv: VecEnv, skip: bool = False):
        assert is_image_space(venv.observation_space) or isinstance(
            venv.observation_space, spaces.dict.Dict
        ), "The observation space must be an image or dictionary observation space"

        self.skip = skip
        # Do nothing
        if skip:
            super(VecNormalizeImage, self).__init__(venv)
            return

        if isinstance(venv.observation_space, spaces.dict.Dict):
            self.image_space_keys = []
            observation_space = deepcopy(venv.observation_space)
            for key, space in observation_space.spaces.items():
                if is_image_space(space):
                    # Keep track of which keys should be transposed later
                    self.image_space_keys.append(key)
                    observation_space.spaces[key] = self.normalize_space(space, key)
        else:
            observation_space = self.normalize_space(venv.observation_space)
        super(VecNormalizeImage, self).__init__(venv, observation_space=venv.observation_space)

    @staticmethod
    def normalize_space(observation_space: spaces.Box, key: str = "") -> spaces.Box:
        """
        Normalize an observation space to -1, 1.

        :param observation_space:
        :param key: In case of dictionary space, the key of the observation space.
        :return:
        """
        # Sanity checks
        assert is_image_space(observation_space), "The observation space must be an image"
        return spaces.Box(low=-1, high=1, shape=observation_space.shape, dtype=np.float32)

    @staticmethod
    def normalize_image(image: np.ndarray) -> np.ndarray:
        """
        Normalize an image or batch of images.

        :param image:
        :return:
        """
        image = image.astype(np.float32)
        image = image / 255
        image = image * 2 - 1
        return image

    def normalize_observations(self, observations: Union[np.ndarray, Dict]) -> Union[np.ndarray, Dict]:
        """
        Normalize (if needed) and return new observations.

        :param observations:
        :return: Transposed observations
        """
        # Do nothing
        if self.skip:
            return observations

        if isinstance(observations, dict):
            # Avoid modifying the original object in place
            observations = deepcopy(observations)
            for k in self.image_space_keys:
                observations[k] = self.normalize_image(observations[k])
        else:
            observations = self.normalize_image(observations)
        return observations

    def step_wait(self) -> VecEnvStepReturn:
        observations, rewards, dones, infos = self.venv.step_wait()

        # Transpose the terminal observations
        for idx, done in enumerate(dones):
            if not done:
                continue
            if "terminal_observation" in infos[idx]:
                infos[idx]["terminal_observation"] = self.normalize_observations(infos[idx]["terminal_observation"])

        return self.normalize_observations(observations), rewards, dones, infos

    def reset(self) -> Union[np.ndarray, Dict]:
        """
        Reset all environments
        """
        return self.normalize_observations(self.venv.reset())

    def close(self) -> None:
        self.venv.close()

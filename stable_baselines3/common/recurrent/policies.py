...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        self.lstm_actor = nn.LSTM(self.features_dim, lstm_hidden_size, num_layers=n_lstm_layers, dropout=lstm_dropout)
        ...
        if self.enable_critic_lstm:
            self.lstm_critic = nn.LSTM(self.features_dim, lstm_hidden_size, num_layers=n_lstm_layers, dropout=lstm_dropout)
    ...
    def forward(
        ...
        latent_pi = self.pi_vf_extractor.forward_actor(latent_pi)
        latent_vf = self.pi_vf_extractor.forward_critic(latent_vf)
    ...
    def get_distribution(
        ...
        latent_pi = self.pi_vf_extractor.forward_actor(latent_pi)
        return self._get_action_dist_from_latent(latent_pi), lstm_states, attn_maps
    ...
    def _predict(
        ...
        """
        distribution, lstm_states, attn_maps = self.get_distribution(observation, lstm_states, episode_starts)
        return distribution.get_actions(deterministic=deterministic), lstm_states, attn_maps
    ...
    def predict(
        ...
        with th.no_grad():
            ...
            actions, states, attn_maps = self._predict(
        ...
        return actions, states, attn_maps

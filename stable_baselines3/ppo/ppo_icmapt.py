...
from stable_baselines3.common.policies import ActorCriticPolicy, APTActorCriticPolicy
...
class PPOAPT(PPO):
    """
    Proximal Policy Optimization algorithm (PPO) (clip version), with ICM-APT
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size, get_next_obs=True):
                ...
                icm_loss = self.policy.update_icm(rollout_data.observations, actions, rollout_data.next_observations)
                self.helper.log({"train/icm_loss": icm_loss}, step=self.helper.state.step)
    ...
    def learn(
        ...
        tb_log_name: str = "PPOAPT",
        eval_log_path: Optional[str] = None,
        reset_num_timesteps: bool = True,
    ) -> "PPOAPT":

        return super(PPOAPT, self).learn(
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            self.helper.log({"train/apt_rew": intrinsic_rews}, step=self.helper.state.step)
            ...
            next_obs_buffer = collections.OrderedDict({k: v for k, v in new_obs.items()})
            for idx, done in enumerate(dones):
                if (
                    ...
                    next_obs_buffer[idx] = self._last_obs[idx]

            rollout_buffer.add(self._last_obs, actions, rewards, self._last_episode_starts, values, log_probs, next_obs=next_obs_buffer)
